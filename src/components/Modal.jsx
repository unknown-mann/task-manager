...
export const Modal = ({ active, setActive }) => {
    ...
    const REPEATING_DAYS = ['mo', 'tu', 'we', 'th', 'fr', 'sa', 'su'];

    const IS_REPEATING_DAYS = {
        "mo": false,
        "tu": false,
        "we": false,
        "th": false,
    ...
    return (
        <ModalEl isActive={active} onClick={() => setActive(false)}>
            <ModalContent isActive={active} onClick={e => e.stopPropagation()}>
                ...
                <SettingButton disabled={isDate} onClick={toggleRepeatStatus}>
                    ...
                    {isRepeat &&
                        <RepeatingDaysWrapper onChange={handleSetRepeatingDays} onClick={e => e.stopPropagation()}>
                            {REPEATING_DAYS.map(day => (
                                <span key={day}>
                                    <DayButton value={day} name={day} id={day} />
                ...
                <SaveButton disabled={!description.trim()} onClick={() => handleAddNewTask()}>SAVE</SaveButton>
            </ModalContent>
        </ModalEl>
    )
}